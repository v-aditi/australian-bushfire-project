...
/**
 * MaxTempChart displays the maximum temperature for the selected location
 * using a line chart.
 */

export default class  MaxTempChart extends React.Component {
    ...
    componentDidMount() {
      /**
       * This function fetches new temperature data when the component first mounts
       */
      var url = 'http://localhost:5000/temperature/<' + this.props.selectedLocation +'>'
        fetch(url)
        .then(res => res.json())
    ...
    componentDidUpdate(prevProps) {
      /**
       * This function fetches new temperature data each time the user selects a new
       * location
    ...
    getLineOptions() {
       ...
       */
        ...
        const dataLocation = 'temp_' + this.props.selectedLocation;     // used to get data from the fetched response

        let tempOption = {
            title: {
              text: 'Maximum Temperature (Degrees Celcius)',
              x: 'center'
            },
            grid: {
              containLabel: true,
